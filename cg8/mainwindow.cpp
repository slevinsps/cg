...
void MyQGraphicsView::ots_rib(int i)
{
    QPen this_pen(otsek_part_color);
    double tmin, tmax, t;
    point_str W, D, N;
    double D_scal, W_scal;
    tmin = 0; tmax = 1;

    D.x = my_edges[i].x2 - my_edges[i].x1;
    D.y = my_edges[i].y2 - my_edges[i].y1;
    for (int j = 0; j < my_otsek.size(); j++)
    {
        W.x = my_edges[i].x1 - my_otsek[j].x1;
        W.y = my_edges[i].y1 - my_otsek[j].y1;

        N.x = -(my_otsek[j].y2 - my_otsek[j].y1);
        N.y = (my_otsek[j].x2 - my_otsek[j].x1);
        point_str check_dir{my_otsek[0].x2 - my_otsek[j].x1, my_otsek[0].y2 - my_otsek[j].y1};
        if (j != my_otsek.size()-1)
        {
            check_dir.x = my_otsek[j+1].x2 - my_otsek[j].x1;
            check_dir.y = my_otsek[j+1].y2 - my_otsek[j].y1;
        }
        int ch_dir = my_scalar(check_dir, N);
        if (ch_dir < 0)
        {
            N.x *= -1;
            N.y *= -1;
        ...
        D_scal = my_scalar(D, N);
        W_scal = my_scalar(W, N);
        if (D_scal == 0)
        {
            if (W_scal < 0)
               return;
    ...
    //qDebug() << "ytlloooooooooooo " << tmin << " "<< tmax;
    if (tmin <= tmax)
    {

        scene->addLine(my_edges[i].x1 + (my_edges[i].x2 - my_edges[i].x1)*tmin,
                       my_edges[i].y1 + (my_edges[i].y2 - my_edges[i].y1)*tmin,
                       my_edges[i].x1 + (my_edges[i].x2 - my_edges[i].x1)*tmax,
                       my_edges[i].y1 + (my_edges[i].y2 - my_edges[i].y1)*tmax,
                       this_pen);
    }
}
...
void MyQGraphicsView::ots_fig()
{
    ...
    qDebug() << my_edges.size();
    qDebug() << my_otsek.size();
    for (int i = 0; i < my_edges.size(); i++)
    {
